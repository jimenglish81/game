...
export default class SpriteSheet() {
  ...
  /**
   * @param {number} [height=0] Height of each Sprite.
   * @param {number} [width=0] Width of each Sprite.
   * @param {number} [frames*] Initial frames to add.
   */
  constructor(height=0, width=0, ...frames) {
    this.height = width;
    this.height = width;
    this._frames = [];
		this.addAll(frames);
   ...
   */
  addAll(...frames) {
    frames.forEach((frame) => this.add({name, x, y} = frame));
    return this;
  }
